				...
				<hlm-popover autoFocus="none" sideOffset="5" [state]="state()" (stateChanged)="stateChanged($event)">
					...
					<hlm-command *hlmPopoverPortal="let ctx" hlmPopoverContent class="p-0" [style.width.px]="triggerWidth()">
						...
						<hlm-command-list autofocus="">
							<hlm-command-group>
								@for (framework of frameworkSelections(); track framework.value) {
...
export class Select39Component {
	...
	protected readonly frameworkSelections = computed(() =>
		this.frameworks.filter((framework) => !this.isSelected(framework)),
	);
