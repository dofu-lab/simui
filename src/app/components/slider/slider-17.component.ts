	...
	template: `
		...
		<div>
			<div class="flex h-12 w-full items-end px-3" aria-hidden="true">
				@for (count of itemCounts; track generateRandomId(); let i = $index) {
					<div class="flex flex-1 justify-center" [style.height]="(count / maxCount) * 100 + '%'">
						<span class="bg-primary/20 size-full"></span>
					</div>
				}
			</div>
			<hlm-slider [min]="0" [max]="900" [(value)]="maxPrice" />
			<div class="mt-4 flex items-center justify-between gap-2">
				<div>
					<span hlmLabel class="mb-2 block">Min price</span>
					<div class="relative">
						<span
				...
				<div>
					<span hlmLabel class="mb-2 block">Max price</span>
					<div class="relative">
						<span
							class="text-muted-foreground pointer-events-none absolute inset-y-0 start-0 flex items-center justify-center ps-3 text-sm peer-disabled:opacity-50">
							$
			...
			<button hlmBtn variant="outline" class="mt-4 w-full">Show {{ computedFilteredItems().length }} results</button>
		</div>
	`,
})
...
export const slider17Code = `import { Component, computed, signal } from '@angular/core';
import { FormsModule } from '@angular/forms';
import { provideIcons } from '@ng-icons/core';
import { lucideRotateCcw } from '@ng-icons/lucide';
import { HlmButton } from '@spartan-ng/helm/button';
